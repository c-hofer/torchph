...
std::tuple<Tensor, Tensor, int> read_profiling_data(){
    ...
    return std::make_tuple(boundary_array, column_dimension, max_dimension);
...
int main()
{
    ...
    auto bm_dim_maxdim = read_profiling_data();

    auto bm = std::get<0>(bm_dim_maxdim);
    auto col_dim = std::get<1>(bm_dim_maxdim);
    auto max_dim = std::get<2>(bm_dim_maxdim);
    ...
    auto ret = calculate_persistence(bm.clone(), col_dim.clone(), max_dim, 10000);
    }
    ProfilerStop();
